...
public class CropView: UIView {
    ...
    @IBInspectable
    public var quadBorderColor = UIColor.blue
    @IBInspectable
    public var quadFillColor = UIColor.clear
    @IBInspectable
    ...
    @IBInspectable
    public var quadBorderWidth:CGFloat = 2.0
    @IBInspectable
    ...
    @IBInspectable
    public var invalidQuadBorderColor = UIColor.red
    public var invalidQuadFillColor = UIColor.red.withAlphaComponent(0.25)
    
    @IBInspectable
    public var showOnlyCorners = false {
        didSet {
            if cropCircles.count > 0 {
                moveNonCornerPoints()
            }
        }
    }
    ...
    public var quad: Quadrilateral {
        guard self.cropCircles.count == 9 else { return Quadrilateral(topLeft: .zero, topRight: .zero, bottomRight: .zero, bottomLeft: .zero) }
//        let pointTransform: (CGPoint)->CGPoint = { CGPoint(x: $0.x - (self.frame.width - self.cropImageView.frame.width) / 2,
//                                                           y: $0.y - (self.frame.height - self.cropImageView.frame.height) / 2) }
        let pointTransform: (CGPoint)->CGPoint = { self.convert($0, to: self.cropImageView) }
        
        let x =  Quadrilateral(topLeft: pointTransform(cropCircles[0].center),
                             topRight: pointTransform(cropCircles[2].center),
                             bottomRight: pointTransform(cropCircles[4].center),
                             bottomLeft: pointTransform(cropCircles[6].center))
            .scale(cropImageView.frame.size, cropImageView.image!.size)
        print(x)
        return x
    }
     ...
     */
    public func setUpImage(image : UIImage, initialQuad: Quadrilateral? = nil){
        self.subviews.forEach { $0.removeFromSuperview() }
        let normImage = normalizedImage(image: image)
        cropImageView = UIImageView(image: normImage)
        self.layoutIfNeeded()
        let frame = AVMakeRect(aspectRatio: normImage.size, insideRect: self.bounds)
        cropImageView.contentMode = .scaleToFill
        cropImageView.frame = frame
        self.addSubview(cropImageView)
        cropFrame = cropImageView.frame
        setUpCropRegion(initialQuad: initialQuad)
...
//        }
//
//        let quad = Quadrilateral(topLeft: corners[0], topRight: corners[1], bottomRight: corners[2], bottomLeft: corners[3])
//
...
//    }
     ...
     */
    private func setUpCropRegion(initialQuad: Quadrilateral?){
        //Add border rectangle layer
        border.fillColor = UIColor.clear.cgColor
        border.lineWidth = quadBorderWidth
        border.strokeColor = quadBorderColor.withAlphaComponent(rectangleAlpha).cgColor
...
//        let points = OpenCVWrapper.getLargestSquarePoints(cropImageView.image, cropImageView.frame.size)
        let points = (initialQuad?.scale(cropImageView.image!.size, cropImageView.frame.size)).flatMap { [$0.topLeft, $0.topRight, $0.bottomRight, $0.bottomLeft] }
        ...
        while(i<=9){
            ...
            case 9:
                cropCircle.center = centerOf(firstPoint: endPoints[0], secondPoint: endPoints[2], thirdPoint: endPoints[1], fourthPoint: endPoints[3])
            default:
                break
            }
 ...
 */
    ...
    @objc internal func panGesture(gesture : UIPanGestureRecognizer){
        ...
        if(gesture.state == UIGestureRecognizer.State.began){
            ...
            for i in stride(from: 1, to: 8, by: 2) {
                let newFrame = CGRect(x: cropCircles[i].center.x - cropCircles[i].frame.width,
                                      y: cropCircles[i].center.y - cropCircles[i].frame.height,
                                      width: cropCircles[i].frame.width * 2,
                                      height: cropCircles[i].frame.height * 2)
            ...
            let newFrame = CGRect(x: cropCircles[8].center.x - cropCircles[8].frame.width,
                                                 y: cropCircles[8].center.y - cropCircles[8].frame.height,
                                                 width: cropCircles[8].frame.width * 2,
                                                 height: cropCircles[8].frame.height * 2)
            if(newFrame.contains(point)) {
                selectedIndex = 8
                oldPoint = point
                cropCircles[selectedIndex!].backgroundColor = selectedCircleBackgroundColor
            ...
            if(selectedIndex == nil || showOnlyCorners){
        ...
        if let selectedIndex = selectedIndex {
            if selectedIndex == 8 {
                ...
                let boundedDeltaX = min(
                    max(cropImageView.frame.origin.x - cropCircles[0].center.x,
                        cropImageView.frame.origin.x - cropCircles[2].center.x,
                        cropImageView.frame.origin.x - cropCircles[4].center.x,
                        cropImageView.frame.origin.x - cropCircles[6].center.x,
                        deltaX),
                    cropImageView.frame.origin.x + cropImageView.frame.width - cropCircles[0].center.x,
                    cropImageView.frame.origin.x + cropImageView.frame.width - cropCircles[2].center.x,
                    cropImageView.frame.origin.x + cropImageView.frame.width - cropCircles[4].center.x,
                    cropImageView.frame.origin.x + cropImageView.frame.width - cropCircles[6].center.x
                )
                let boundedDeltaY = min(
                    max(cropImageView.frame.origin.y - cropCircles[0].center.y,
                        cropImageView.frame.origin.y - cropCircles[2].center.y,
                        cropImageView.frame.origin.y - cropCircles[4].center.y,
                        cropImageView.frame.origin.y - cropCircles[6].center.y,
                        deltaY),
                    
                    cropImageView.frame.origin.y + cropImageView.frame.height - cropCircles[0].center.y,
                    cropImageView.frame.origin.y + cropImageView.frame.height - cropCircles[2].center.y,
                    cropImageView.frame.origin.y + cropImageView.frame.height - cropCircles[4].center.y,
                    cropImageView.frame.origin.y + cropImageView.frame.height - cropCircles[6].center.y
                )
                var newCenters = [CGPoint]()
                for i in stride(from: 0, through: 6, by: 2) {
                    newCenters.append(CGPoint(x: cropCircles[i].center.x + boundedDeltaX,
                                              y: cropCircles[i].center.y + boundedDeltaY))
                }
                oldPoint = point//CGPoint(x: point.x + boundedDeltaX,
                                    //y: point.y + boundedDeltaY)
                for i in 0...3 {
                    cropCircles[2*i].center = newCenters[i]
                }
                ...
                if(isInsideFrame(pt: pt1New) && isInsideFrame(pt: pt2New)) && pt1New != pt2New {
                    pt1.center = pt1New
                    pt2.center = pt2New
                }
                ...
                if cropCircles.enumerated()
                    .allSatisfy({ index, circle in
                        index == selectedIndex || circle.center != finalPoint  }) {
    ...
    private func moveNonCornerPoints(){
        for i in stride(from: 1, to: 8, by: 2){
            ...
            cropCircles[i].isHidden = showOnlyCorners
        }
        cropCircles[8].center = centerOf(firstPoint: cropCircles[0].center, secondPoint: cropCircles[4].center, thirdPoint: cropCircles[2].center, fourthPoint: cropCircles[6].center)
        cropCircles[8].isHidden = showOnlyCorners
    }
    ...
    private func checkQuadrilateral(){
        ...
        if(checkIfOppositeSides(p1: B,p2: D,l1: A,l2: C) && checkIfOppositeSides(p1: A,p2: C,l1: B,l2: D)){//Convex
            border.strokeColor = quadBorderColor.cgColor
            border.fillColor = self.quadFillColor.cgColor
        }else if(!checkIfOppositeSides(p1: B,p2: D,l1: A,l2: C) && !checkIfOppositeSides(p1: A,p2: C,l1: B,l2: D)){//Complex
            border.strokeColor = quadBorderColor.cgColor
            border.fillColor = self.quadFillColor.cgColor
            reorderEndPoints()
        } else{//Concave
            border.strokeColor = self.invalidQuadBorderColor.cgColor
            border.fillColor = self.invalidQuadFillColor.cgColor
        }
    }
    ...
    private func centerOf(firstPoint: CGPoint, secondPoint: CGPoint, thirdPoint: CGPoint, fourthPoint: CGPoint) -> CGPoint {
        return CGPoint(x: (firstPoint.x + secondPoint.x + thirdPoint.x + fourthPoint.x) / 4,
                       y: (firstPoint.y + secondPoint.y + thirdPoint.y + fourthPoint.y) / 4)
//        let denominator = (firstPoint.x - secondPoint.x)*(thirdPoint.y - fourthPoint.y) - (firstPoint.y - secondPoint.y)*(thirdPoint.x - fourthPoint.x)
//        guard denominator != 0 else { return .zero }
//
//        return CGPoint(x: ((firstPoint.x*secondPoint.y - firstPoint.y*secondPoint.x)*(thirdPoint.x - fourthPoint.x) - (firstPoint.x - secondPoint.x)*(thirdPoint.x*fourthPoint.y - thirdPoint.y*fourthPoint.x))/denominator,
//                       y: ((firstPoint.x*secondPoint.y - firstPoint.y*secondPoint.x)*(thirdPoint.y - fourthPoint.y) - (firstPoint.y - secondPoint.y)*(thirdPoint.x*fourthPoint.y - thirdPoint.y*fourthPoint.x))/denominator)
    }
